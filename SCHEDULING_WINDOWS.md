...
## 1. Create the two scheduled tasks
...
schtasks /create /tn "SelfEvolvingWorld-Simulate" /tr "`"$python`" `"$world\driver.py`" simulate" /sc minute /mo 15 /st 00:00 /f
...
## 2. Test each one manually before trusting the schedule
